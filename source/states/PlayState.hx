...
class PlayState extends MusicBeatState
{
	...
	var invertStrum:Bool = false;
	var opponentStrum:Bool = true;
	var cameraForce:Float = 10;
	public var visibleDad:Bool = true;
	var scorePopUpText:FlxText;
	override public function create()
	{
		...
		invertStrum = stageData.invert_strum;

		if (stageData.camera_force != null)
			cameraForce = stageData.camera_force;

		if(stageData.camera_speed != null)
			cameraSpeed = stageData.camera_speed;

		opponentStrum = stageData.opponent_strum;
		if (stageData.opponent_strum == null)
			opponentStrum = ClientPrefs.data.opponentStrums;
		...
		timeTxt.setFormat(Paths.font("DroidSerif-Regular.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		healthBar.leftToRight = invertStrum;
		...
		add(healthBar);
		if (!visibleDad){
			var sameColor:FlxSprite = invertStrum ? healthBar.rightBar : healthBar.leftBar;
			sameColor.color = !invertStrum ? healthBar.rightBar.color : healthBar.leftBar.color;
		}

		//invertStrum
		var iconHealth = [
			boyfriend.healthIcon,
			dad.healthIcon,
		];
		iconP1 = new HealthIcon(iconHealth[!invertStrum ? 0 : 1], true);
		iconP1.y = healthBar.y - 75;
		iconP1.visible = !ClientPrefs.data.hideHud;
		iconP1.alpha = ClientPrefs.data.healthBarAlpha;
		add(iconP1);

		iconP2 = new HealthIcon(iconHealth[invertStrum ? 0 : 1], false);
		...
		if (!ClientPrefs.data.hideHud){
			var iconVisible:HealthIcon = iconP2;
			if (invertStrum) iconVisible = iconP1;
			iconVisible.visible = visibleDad;
		}

		scoreTxt = new FlxText(0, healthBar.y + 40, FlxG.width, "", 20);
		scoreTxt.setFormat(Paths.font("DroidSerif-Regular.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("DroidSerif-Regular.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scorePopUpText = new FlxText(0, 0, 0, "", 32);
		scorePopUpText.setFormat(Paths.font("DroidSerif-Regular.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scorePopUpText.scrollFactor.set();
		scorePopUpText.y = healthBar.y - 60;
		// scorePopUpText.x -= 30;
		scorePopUpText.visible = !ClientPrefs.data.hideHud;
		add(scorePopUpText);
	...
	public function reloadHealthBarColors() {
		// invertStrum
		var colorHealth:Array<FlxColor> = [
			FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]),
			FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2])
		];

		healthBar.setColors(colorHealth[!invertStrum ? 0 : 1], colorHealth[invertStrum ? 0 : 1]);
	}
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer)
			{
				...
				notes.forEachAlive(function(note:Note) {
					if(opponentStrum || note.mustPress)
					{
	...
	function startSong():Void
	{
		...
		#if desktop
		// Updating Discord Rich Presence (with Time Left)]v
		var iconDiscord:String = (!invertStrum) ? iconP2.getCharacter() : iconP1.getCharacter();
		// (!invertStrum) ?
		DiscordClient.changePresence(detailsText, SONG.song, iconDiscord, true, songLength);
		#end
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		for (i in 0...4)
		{
			...
			if (player < 1)
			{
				if(!opponentStrum) targetAlpha = 0;
	...
	function invertStrumNote(invert:Bool) {
		if (invert){
			...
			var bullShit:Int = 0;
			for (playerStrum in playerStrums.members){
				playerNote.push(playerStrum.x);
				playerStrum.x = opponentNote[bullShit].x;

				bullShit++;
			}

			bullShit = 0;
			for (opponetStrum in opponentStrums.members){
				opponetStrum.x = playerNote[bullShit];
	...
	override public function onFocusLost():Void
	{
		#if desktop
		var iconDiscord:String = (!invertStrum) ? iconP2.getCharacter() : iconP1.getCharacter();
		if (health > 0 && !paused) DiscordClient.changePresence(detailsPausedText, SONG.song, iconDiscord);
		#end
	...
	function resetRPC(?cond:Bool = false)
	{
		...
		if (cond)
			DiscordClient.changePresence(detailsText, SONG.song, iconDiscord, true, songLength - Conductor.songPosition - ClientPrefs.data.noteOffset);
		else
			DiscordClient.changePresence(detailsText, SONG.song, iconDiscord);
		#end
	}
	...
	override public function update(elapsed:Float)
	{
		...
		iconP1.animation.curAnim.curFrame = (rulesIcons[0]) ? 1 : 0;
		iconP2.animation.curAnim.curFrame = (rulesIcons[1]) ? 1 : 0;
	...
	function openPauseMenu()
	{
		...
		#if desktop
		var iconDiscord:String = (!invertStrum) ? iconP2.getCharacter() : iconP1.getCharacter();
		DiscordClient.changePresence(detailsPausedText, SONG.song, iconDiscord);
		#end
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("Game Over - " + detailsText, SONG.song, iconDiscord);
				#end
				isDead = true;
				return true;
			}
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName) {
			...
			case 'Flash Camera':
				var cameraValue:String = value2.split(', ')[0].toLowerCase();
				var colorValue:String = value2.split(', ')[1].toLowerCase();
				if (flValue1 == null) flValue1 = 0.5;

				var cameraFlash:FlxCamera;
				var colorFlash:FlxColor;
				trace('Flash Camera, ' + cameraValue);
				trace('Color Camera, ' + colorValue);

				switch(cameraValue.trim()){
					case 'hud' | 'camhud':
						cameraFlash = camHUD;
					case 'other' | 'camother':
						cameraFlash = camOther;
					default:
						cameraFlash = camGame;
				}

				switch(colorValue.trim()){
					case 'white' | 'w':
						colorFlash = FlxColor.WHITE;
					default:
						colorFlash = FlxColor.BLACK;
				}

				if (cameraFlash != camGame)
					cameraFlash.flash(colorFlash, flValue1);
				else {
					var blcTheme:FlxSprite = new FlxSprite();
					blcTheme.makeGraphic(Std.int(cameraFlash.viewWidth),Std.int(cameraFlash.viewHeight),colorFlash);
					blcTheme.scrollFactor.set();
					blcTheme.screenCenter();
					// blcTheme.setGraphicSize(Std.int(blcTheme.width/defaultCamZoom),Std.int(blcTheme.height/defaultCamZoom));
					// blcTheme.updateHitbox();
					add(blcTheme);
					blcTheme.alpha = 1;
					// defaultCamZoom

					FlxTween.tween(blcTheme, {alpha: 0}, flValue1);
				}
		}
	...
	function moveCameraSection(?sec:Null<Int>):Void {
		...
		if (gf != null && SONG.notes[sec].gfSection)
		{
			...
			tweenCamIn();
			stagesFunc(function(stage:BaseStage) stage.onMoveCamera('gf'));
			callOnScripts('onMoveCamera', ['gf']);
			cameraCharacters[2] = [camFollow.x,camFollow.y];
			return;
		}
	...
	public function addCinematicBar() {
		for (i in 0...2){
			var bartop:FlxSprite = new FlxSprite(0,620 * i).makeGraphic(FlxG.width, 100,FlxColor.BLACK);
	...
	// bf -- dad -- gf
	var cameraCharacters:Array<Array<Float>> = [];
	public function moveCamera(isDad:Bool)
	{
		if(isDad)
		{
			...
			stagesFunc(function(stage:BaseStage) stage.onMoveCamera('dad'));
			cameraCharacters[1] = [camFollow.x,camFollow.y];
			tweenCamIn();
		}
		else
		{
			...
			cameraCharacters[0] = [camFollow.x,camFollow.y];
			stagesFunc(function(stage:BaseStage) stage.onMoveCamera('boyfriend'));
	...
	var ratingTweenZoom:FlxTween = null;
	var ratingTween:FlxTween = null;
	// var rating:FlxTimer = null;
	private function popUpScore(note:Note = null):Void
	{
		...
		var ratinNameText:String = daRating.name;

		if (ratinNameText == 'sick'){
			ratinNameText = 'Sick!';
		} else {
			var ratinNameTextPOS:String = '';
			for (i in 0...ratinNameText.split('').length){
				var curLetterRating = ratinNameText.split('')[i];
				var letterPOS:String = curLetterRating;
				if (i == 0){
					letterPOS = letterPOS.toUpperCase();
				}

				ratinNameTextPOS += letterPOS;
			}
			ratinNameText = ratinNameTextPOS;
		}


		scorePopUpText.text = ratinNameText + ' (x${combo})';
		scorePopUpText.screenCenter(X);
		scorePopUpText.alpha = 1;

		if(ratingTween != null) ratingTween.cancel();


		ratingTween = FlxTween.tween(scorePopUpText, {alpha: 0}, 0.5,{startDelay: 1, onComplete: function(t:FlxTween){
			ratingTween = null;
		}});

		if(ClientPrefs.data.scoreZoom && !cpuControlled){
			if(ratingTweenZoom != null) 
				ratingTweenZoom.cancel();
			

			scorePopUpText.scale.x = 1.2;
			ratingTweenZoom = FlxTween.tween(scorePopUpText.scale, {x: 1}, 0.3, {ease: FlxEase.circOut, onComplete: function(t:FlxTween){
				ratingTweenZoom = null;
			}});
		}
	}
	...
	function cameraNoteMove(hitSection:Bool,gfSection:Bool){
		...
		var gfSing:Bool = cameraCharacter == gf;
		var dad:Bool = cameraCharacter == dad;

		var defaultCamera:Array<Float> = cameraCharacters[gfSing ? 2 : (dad ? 1 : 0)];
		var camMov:FlxPoint = new FlxPoint(defaultCamera[0],defaultCamera[1]);
		switch(cameraCharacter.animation.curAnim.name){
			case 'singUP':
				camMov.y -= cameraForce;
			case 'singDOWN':
				camMov.y += cameraForce;
			case 'singLEFT':
				camMov.x -= cameraForce;
			case 'singRIGHT':
				camMov.x += cameraForce;
		}

		camFollow.setPosition(camMov.x,camMov.y);
	}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			if(char != null)
			{
				char.playAnim(animToPlay, true);
				char.holdTimer = 0;
				// cameraNoteMove(char/*, true, note.gfNote*/);
			}
		}
		...
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				...
				if(char != null)
				{
					...
					char.holdTimer = 0;
					// cameraNoteMove(char/*, false, note.gfNote*/);
